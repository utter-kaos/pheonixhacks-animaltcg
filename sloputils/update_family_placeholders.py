...
def update_family_placeholders():
    ...
    try:
        ...
        for card_id, family, attack, attack_description in cards:
            ...
            cur.execute(
                "UPDATE cards SET attack = ?, attack_description = ? WHERE id = ?",
                (updated_attack, updated_description, card_id)
            )

        conn.commit()
        print("Family placeholders updated successfully.")
